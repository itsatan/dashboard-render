...
【叶子组件 - 无 children】4、5、6、7、8、9、10、11、12
...
5. MetricCard/LineChart/BarChart/PieChart/RadarChart/GaugeChart/ChartPlaceholder/SectionTitle/StatusDot 是叶子组件，没有 children
6. 每个元素必须有 type 和 props，容器组件还需要 children
7. 根据内容特征灵活决定布局，追求视觉平衡与美观
8. 单个图表直接放入父容器，不要用 Flex direction="row" 包裹
9. 需要全宽展示的图表直接放入 Flex direction="col" 父容器中
...
❌ MetricCard与图表混排
❌ 小图表独占一行
❌ Flex direction="row" 只放1个元素（单个图表直接放入父容器即可）
❌ 用Flex row包裹单个图表
...
| 关键词 | 推断需求 |
|--------|----------|
| 监控、状态、健康 | MetricCard + GaugeChart + LineChart |
| 对比、分析、比较 | BarChart + PieChart |
| 趋势、变化、历史 | LineChart |
| 占比、分布、构成 | PieChart |
| 评估、画像、能力 | RadarChart |
| 进度、完成率 | GaugeChart |
...
8. 每个仪表盘必须包含且仅包含一个 SectionTitle
在输出前必须自检以上规则。
